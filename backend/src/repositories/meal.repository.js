...
class MealRepository extends BaseRepository {
   ...
   */
  async create(data) {
    const { userId, date, mealType, transcript, foods, total_calories } = data;
    const result = await this._query(
      MEAL_QUERIES.CREATE,
      [userId, date, mealType, transcript, JSON.stringify(foods), total_calories],
    );
  ...
  async update(id, fields) {
    const { meal_type, foods, total_calories, date } = fields;
    const result = await this._query(
      MEAL_QUERIES.UPDATE,
      [meal_type, foods ? JSON.stringify(foods) : null, total_calories, date, id],
    );
